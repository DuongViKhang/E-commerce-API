        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">DataTransaction</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">DataTable of Transaction</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <c:forEach var="i" begin="0" end="${size-1}">
                      <tr>
                      		<td>${listtr[i].getTransactionId()}</td>
                      	  <td>
                      	  	<c:forEach var="k" items="${lists}">
                            	<c:if test="${listtr[i].getStoreId() == k.storeId}">${k.storeName}</c:if>
                            </c:forEach>
                      	  </td>
                      	  <%-- <td>${listtr[i].getStoreId()}</td> --%>
                      	  <td>
                      	  	<c:forEach var="o" items="${listu}">
                            	<c:if test="${listtr[i].getUserId() == o.userId}">${o.userName}</c:if>
                            </c:forEach>
                      	  </td>
                          <%-- <td>${listtr[i].getUserId()}</td> --%>
                          <td>${listtr[i].getAmount()}</td>
                          <td>${listtr[i].getDate()}</td>
                      </tr>
                  </c:forEach>
                  </tbody>
                  <tfoot></tfoot>
