              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <c:forEach var="i" begin="0" end="${size-1}">
                      <tr>
                      	  <td>${listo[i].getOrderId()}</td>
                      	  <td>
                      	  	<c:forEach var="k" items="${listu}">
                            	<c:if test="${listo[i].getUserId() == k.userId}">${k.userName}</c:if>
                            </c:forEach>
                      	  </td>
                      	  
                      	  <%-- <td>
                      	  	<c:forEach var="k" items="${listot}">
                            	<c:if test="${listo[i].getOrderId() == k.orderId}">${k.productId}</c:if>
                            </c:forEach>
                      	  </td>
                      	  <td>
                      	  	<c:forEach var="o" items="${listot}">
                            	<c:if test="${listo[i].getOrderId() == o.orderId}">${o.count}</c:if>
                            </c:forEach>
                      	  </td> --%>
                      	  <td>${listo[i].getAmountFromUser()}</td>
                      	  <td>${listo[i].getDate()}</td>
                      	  <td>
                      	  	<c:if test="${listo[i].getStatusId() == 1}">
                      	  		<a href="http://localhost:8080/Shopee/seller/InvoiceControl?OId=${listo[i].getOrderId()}">Đang xử lý</a>
                      	  	</c:if>
                      	  	<c:if test="${listo[i].getStatusId() == 2}">
                      	  		<a href="http://localhost:8080/Shopee/seller/InvoiceControl?OId=${listo[i].getOrderId()}" Style="color:green;">Đang Giao Hàng</a>
                      	  	</c:if>
                      	  </td>
                      </tr>
                  </c:forEach>
